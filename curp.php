<?php
if(strpos($message, "/curp")===0 or strpos($message, "!curp")===0 or strpos($message, ".curp")===0){

sendaction($chatId, typing);
deleteprm($userId);
$Comu = substr($message, 6);
if (empty($Comu)){
        reply_to($chatId, $message_id,$keyboard,'<b>Ingresa una CURP:</b>');
        die();
    }
...
$get = file_get_contents("http://192.162.69.227:8081/curp/".trim($Comu)."/ritaGod@");
...
if (empty($nombre)){
        reply_to($chatId, $message_id,$keyboard,'<b>Datos no encontrados</b>');
        die();
    }
